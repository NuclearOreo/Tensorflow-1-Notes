...
hidden_layer = tf.layers.dense(X, num_input, activation=tf.nn.relu, kernel_initializer=initializer)

logits = tf.layers.dense(hidden_layer, num_output)
...
gradient_and_variable = optimizer.compute_gradients(cross_entropy)

gradients = []
gradients_placeholders = []
grads_and_vars_feed = []

for gradient, variable in gradient_and_variable:
    gradients.append(gradient)
    gradients_placeholder = tf.placeholder(tf.float32, shape=gradient.get_shape())
